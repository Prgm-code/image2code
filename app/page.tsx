...
export default function Home() {
	...
	const trasformImageToCode = async (file: File) => {
  ...
  const transImageformToCode = async (file: File) => {
	...
	return (
		<div className="grid grid-cols-[400px_1fr]">
			<aside className="flex flex-col justify-between min-h-screen p-4 bg-gray-900">
				...
				<section>{/* aqui iran los filtro */} </section>
				<footer>Desarrolado por PRGM-Code</footer>
			</aside>
				...
				<section className="max-w-5xl mx-auto p-10">
					...
					{step === STEPS.INITIAL && (
						<div className="flex flex-col gap-4">
							<DragAndDrop transImageformToCode={transImageformToCode} />
							<Form transformUrltoCode={transformUrltoCode} />
						</div>
					)}
					{step === STEPS.PREVIEW && (
						<div className="rounded flex flex-col gap-4">
							<iframe srcDoc={result} className="w-full h-full border-4 rounded border-gray-700 aspect-square" />
							<pre className="pt-10">
								<code>{result}</code>
							</pre>
						</div>
